...
def convert_to_image(path):
    ...
    current_total_revenue_field, previous_total_revenue_field = extract_cash_values(
        total_revenue, check=False
    )

    current_uk_revenue_field, previous_uk_revenue_field = extract_cash_values(
        uk_revenue, check=False
    )

    current_total_turnover_field, previous_total_turnover_field = extract_cash_values(
        total_turnover, check=False
    )

    current_uk_turnover_field, previous_uk_turnover_field = extract_cash_values(
        uk_turnover, check=False
    )

    current_vat_debtor_field, previous_vat_debtor_field = extract_cash_values(
        vat_debtor, check=True
    )
    current_vat_creditor_field, previous_vat_creditor_field = extract_cash_values(
        vat_creditor, check=True
    )
    ...
    try:
        if current_uk_revenue_field and previous_uk_revenue_field:
            current_overseas_revenue_field = abs(
                current_total_revenue_field - current_uk_revenue_field
            )
            previous_overseas_revenue_field = abs(
                previous_total_revenue_field - previous_uk_revenue_field
            )
        else:
    ...
    try:
        if current_uk_turnover_field and previous_uk_turnover_field:
            current_overseas_turnover_field = abs(
                current_total_turnover_field - current_uk_turnover_field
            )
            previous_overseas_turnover_field = abs(
                previous_total_turnover_field - previous_uk_turnover_field
            )
        else:
    ...
    final_details = {
        "uk_revenue_current_year": [current_uk_revenue_field],
...
def extract_vat_debtor_from_data(data):
    ...
    if not debtor_index.empty and not creditor_index.empty:
        ...
        if not vat_debt.empty:
            vat_df = vat_debt.loc[
                vat_debt["text"].str.contains(r"(?:VAT)", regex=True) == True
            ]
            if not vat_df.empty:
...
def extract_vat_creditor_from_data(data):
    ...
    if not creditor_index.empty:
        ...
        if not vat_cred.empty:
            vat_df = vat_cred.loc[
                vat_cred["text"].str.contains(r"(?:VAT)", regex=True) == True
            ]
            if not vat_df.empty:
                vat_line = vat_df["line_num"].iloc[-1]
                vat = data.loc[data["line_num"] == vat_line]
